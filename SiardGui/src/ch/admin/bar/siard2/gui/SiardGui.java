 ...
 * ------------------------------------------------------------------------ Copyright : Swiss
 * Federal Archives, Berne, Switzerland, 2017 Created : 05.05.2017, Hartwig Thomas, Enter AG, Rüti
 * ZH ======================================================================
 */
...
import ch.admin.bar.siard2.gui.actions.ExportAction;
import ch.admin.bar.siard2.gui.actions.FindAction;
import ch.admin.bar.siard2.gui.actions.InstallUninstallHandler;
import ch.admin.bar.siard2.gui.actions.MetaDataAction;
import ch.admin.bar.siard2.gui.actions.OpenSaveAction;
...
import ch.enterag.utils.fx.tasks.SleeperTask;
...
/* ==================================================================== */
/**
 * SiardGui implements the graphical user interface of SIARD Suite 2.x as a JavaFX application.
 *
...
public class SiardGui extends Application
	...
	public static List<String> getProvenances() {
	...
	public Scene getScene() {
		return (_stage == null) ? null : _stage.getScene();
	}
	 ...
	 * @return 1 for running version greater than given version (or given version is null), 0 for
	 *         running version equals given version, -1 for running version is less than given version
	 *         (not null).
	 */
	public static int compareVersion(String sVersion) {
		...
		if (sVersion != null) {
			...
			for (int iVersionComponent = 0; (iCompare == 0) && (iVersionComponent < Math.min(asInstalled.length, asRunning.length)); iVersionComponent++)
	...
	/*------------------------------------------------------------------*/
	/**
	 * checks whether the current instance is running from a JAR installed in the given folder.
	 *
	 * @param fileInstalled folder where SIARD Suite is installed.
	 * @return true, if current instance is running from JAR installed in the given folder.
	 */
	...
	/*------------------------------------------------------------------*/
	/**
	 * check if running instance is installed instance. N.B.: Otherwise UserProperties are not stored.
	 *
	...
	/*------------------------------------------------------------------*/
	/**
	 * setting the language to a different values changes the whole user interface.
	 *
	...
	public void findNext() {
		...
		try {
			ms = getArchive().getMetaData().findNext(du);
		} catch (IOException ie) {
			_il.exception(ie);
		}
		if (ms != null)
	...
	/*------------------------------------------------------------------*/
	/**
	 * collapse currently selected record extract and its parents up to the rows entry of the table tree
	 * item.
	 */
	...
	/*------------------------------------------------------------------*/
	/**
	 * canClose calls the handler of the CloseRequest in order to treat internal close attempts like
	 * external ones.
	 *
	...
	/*------------------------------------------------------------------*/
	/**
	 * compare current version with installed version and offer installation if the current version is
	 * new or more recent.
	 *
	 * @return
	 */
	...
	public void start(Stage stage) throws Exception {
		...
		SleeperTask.runSleeperTask(iSplashMs, new EventHandler<WorkerStateEvent>() {
			/*----------------------------------------------------------------*/
			/**
			 * this is called after the splash panel has terminated. It finishes the start() activities.
			 */
			@Override
	...
	/*------------------------------------------------------------------*/
	/**
	 * init is called by launcher before start. initialize open directory.
	 */
	@Override
	public void init()
		throws Exception {
	 ...
	 * make log directory
	 *
	 * logging.properties의 "java.util.logging.FileHandler.pattern" 값을 읽어 log 디렉토리 생성
	 */
	public void makeLogDir() {
		...
		try {
			FileReader resources = new FileReader("etc/logging.properties");
			properties.load(resources);

			logFolder = properties.getProperty("java.util.logging.FileHandler.pattern");

			if (logFolder != null) {
				logFolder = logFolder.substring(0, logFolder.lastIndexOf("/"));

				File logDir = new File(logFolder);

				if (!logDir.exists()) { // log 디렉토리 없으면 생성.
					System.out.println("make log directory : " + logFolder);
					_il.info("make log directory : " + logFolder);
					logDir.mkdirs();
				}
			}
		} catch (IOException e) {
