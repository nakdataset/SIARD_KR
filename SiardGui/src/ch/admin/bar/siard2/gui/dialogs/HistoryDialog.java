...
public class HistoryDialog extends ScrollableDialog implements EventHandler<ActionEvent> {
	...
	private HistoryDialog(Stage stageOwner) {
		...
		//TODO 최창근 추가 - 사용자 이벤트 내역 select Test
		CommonDAO dao = new CommonDAO();
		try {
			LOG.info("resultList " + dao.selectListHistory().toString());
		} catch (Exception e) {
	...
	/*------------------------------------------------------------------*/
	/**
	 * create the VBox containing the title History.
	 *
	 * @return VBox with title information.
	 */
	private VBox createVBoxTitle() {
		LOG.info("createVBoxTitle");
	...
	private HBox createHBoxCredits(Label lblCredits, Text txtCredits) {
		...
		if (txtCredits != null) {
			...
			String cssLayout = "-fx-border-color: green;\n" +
	            ...
	            "-fx-border-width: 3;\n" +
	            "-fx-border-style: dashed;\n";
			hboxCredits.setStyle(cssLayout);
		}
		return hboxCredits;
	} /* createHBoxCredits */
	...
	private VBox createVBoxCredits() {
		...
		String cssLayout = "-fx-border-color: orange;\n" +
            ...
            "-fx-border-width: 3;\n" +
            "-fx-border-style: dashed;\n";
		vboxCredits.setStyle(cssLayout);
	...
	public static void showHistoryDialog(Stage stageOwner) {
		LOG.info("showInfoDialog");
		HistoryDialog id = new HistoryDialog(stageOwner);
