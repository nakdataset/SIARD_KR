...
/* ==================================================================== */
/**
 * a table view with a list of objects per row. - displays and aligns objects according to their
 * type. - generates an action event ("show cell") for double clicks or enter keys on a cell. The
 * target of the action event is an ObjectTableCell with information about row and column index.
 *
	...
	extends TableView<List<Object>>
{
			...
			while (change.next()) {
				if (change.wasRemoved()) {
					if (change.getRemovedSize() == 1) {
						...
						// "+String.valueOf(tp.getRow())+","+String.valueOf(tp.getColumn()));
					} else
	...
	private TextField select(ObjectTableCell otc, int iStartOffset, int iEndOffset)
	{
		...
		// System.out.println("Size before deselect:
		// "+String.valueOf(getSelectionModel().getSelectedCells().size()));
		for (int iSelected = 0; iSelected < getSelectionModel().getSelectedCells().size(); iSelected++) {
			...
			// System.out.println("deselect["+String.valueOf(iSelected)+"]:
			// "+String.valueOf(tp.getRow()+","+tp.getColumn()));
			getTableCell(tp.getRow(), tp.getColumn()).deselect();
			// System.out.println("Size after single deselect:
			// "+String.valueOf(getSelectionModel().getSelectedCells().size()));
			// getSelectionModel().clearSelection(tp.getRow(), tp.getTableColumn());
		}
		getSelectionModel().clearSelection();
		// System.out.println("Size after deselect:
		// "+String.valueOf(getSelectionModel().getSelectedCells().size()));
		if (otc != null) {
			otc.setFocus();
	...
	private EventHandler<MouseEvent> _mef = new EventHandler<MouseEvent>() {
		/*----------------------------------------------------------------*/
		/**
		 * mouse event on the way to text field is redirected to containing table cell.
		 */
		@Override
	...
	private EventHandler<MouseEvent> _meh = new EventHandler<MouseEvent>() {
		/*----------------------------------------------------------------*/
		/**
		 * mouse event from table cell translates double click events to show cell events.
		 */
		@Override
		public void handle(MouseEvent me)
		{
	...
	private EventHandler<KeyEvent> _keh = new EventHandler<KeyEvent>() {
		/*------------------------------------------------------------------*/
		/**
		 * key event from table cell translates enter key pressed events to show cell events and moves the
		 * focus of cursor keys.
		 */
		@SuppressWarnings("rawtypes")
		@Override
		...
		extends TableCell<List<Object>, Object>
	{
		...
		private void addTableCell()
		{
			if (getRow() < getItems().size()) {
				...
				// System.out.println("Added TableCell "+getId()+" on "+getRow()+"/"+getColumn()+" minheight:
				// "+getMinHeight());
		...
		/*----------------------------------------------------------------*/
		/*
		 * remove mouse and keyboard listener from columns 0 and color it gray.
		 * 
		...
		/*----------------------------------------------------------------*/
		/**
		 * updateItem converts item to string and sets alignment. Also, the row and column is determined.
		 * 
		 * @param oItem is a pair of a List<Object> record and an object list table column constructed by
		 *        the cell value factory below.
		...
		public void updateItem(Object oItem, boolean bEmpty)
		{
			if (oItem != null) {
				...
				if (!bEmpty) {
					...
					_tctf.setText(sText);
					setGraphic(_tctf);
					// System.out.println("Updated TableCell "+getId()+" on "+getRow()+"/"+getColumn()+" minheight:
					// "+getMinHeight());
				}
			}
		} /* updateItem */
		...
		/*----------------------------------------------------------------*/
		/**
		 * set the focus to this table cell. (The focus within the table view is managed by the table view.)
		 */
	...
	/*------------------------------------------------------------------*/
	/**
	 * get scroll bar of the given orientation from the skin. N.B.: Java 9/10 makes it virtually
	 * impossible to use an external JAVA 1.8 implementation of JavaFX!
	 * com.sun.javafx.scene.control.skin and javafx.scene.control.skin clash!!
	 * 
	...
	/*------------------------------------------------------------------*/
	/**
	 * initiates computing of table height and setting of minHeight, prefHeight, maxHeight. (Listen to
	...
	{
		/*
		 * make the desired cell visible (and thus make sure, it is updated and added to in _llTableCells)
		 */
		scrollTo(iRow);
